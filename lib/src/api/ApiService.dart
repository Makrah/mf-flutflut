...
class ApiService {
  ApiService() {
    ...
    dio.interceptors
      ...
      final String storageToken = await LocalStorageService.get(StorageKeys.token);
  ...
  Future<LoginResponseDto> signup(LoginDto body) async {
    final Response<Map<String, dynamic>> response = await dio.post('/user/signup', data: body.toJson());
    return LoginResponseDto.fromJson(response.data);
  }

  // User
  Future<UserResponseDto> patchUser(UpdateUserDto body) async {
    final Response<Map<String, dynamic>> response = await dio.patch('/user', data: body.toJson());
  ...
  Future<UserResponseDto> getUser(String userId) async {
    final Response<Map<String, dynamic>> response = await dio.get('/user/$userId');
    return UserResponseDto.fromJson(response.data);
  }

  // Post
  Future<PostResponseDto> createPost(CreatePostDto body) async {
    final Response<Map<String, dynamic>> response = await dio.post('/post', data: body.toJson());
    return PostResponseDto.fromJson(response.data);
  }

  Future<PostResponseDto> getPost(String postId) async {
    final Response<Map<String, dynamic>> response = await dio.get('/post/$postId');
    return PostResponseDto.fromJson(response.data);
  }
  ...
  Future<CommentResponseDto> commentOnPost(
    ...
    final Response<Map<String, dynamic>> response = await dio.post('/post/$postId/comment', data: body);
  ...
  Future<List<PostDto>> getPostByPosition(int lat, int long, int radius) async {
    ...
    return (list as List<Map<String, dynamic>>).map((Map<String, dynamic> e) => PostDto.fromJson(e)).toList();
  }
}
